...
export default function ReportsPage() {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          <KpiCard label="Jami o'quvchi" value={data.total || 0} color="#0F172A" borderColor="#E2E8F0" />
          <KpiCard label="Keldi" value={data.present || 0} color="#059669" borderColor="#A7F3D0" />
          <KpiCard label="Kech keldi" value={data.late || 0} color="#D97706" borderColor="#FDE68A" />
          <KpiCard label="Kelmadi" value={data.absent || 0} color="#DC2626" borderColor="#FECACA" />
        </div>
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>Davomat ko'rsatkichi</p>
              <p style={{ fontSize: 12, color: '#94A3B8', marginTop: 2 }}>Bugungi umumiy holat</p>
            </div>
            <span style={{
        ...
        {classData.length > 0 && (
          <div style={cardStyle}>
            <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', marginBottom: 18 }}>Sinflar bo'yicha taqqoslama</p>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={classData} barCategoryGap="25%">
                ...
                <Bar dataKey="Keldi" fill="#10B981" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Kech keldi" fill="#F59E0B" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Kelmadi" fill="#EF4444" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
  ...
  const renderWeekly = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          <KpiCard label="Keldi" value={ov.present || 0} color="#059669" borderColor="#A7F3D0" />
          <KpiCard label="Kech keldi" value={ov.late || 0} color="#D97706" borderColor="#FDE68A" />
          <KpiCard label="Kelmadi" value={ov.absent || 0} color="#DC2626" borderColor="#FECACA" />
          <KpiCard label="Ko'rsatkich" value={`${ov.attendance_rate || 0}%`} color="#4F46E5" borderColor="#C7D2FE" />
        </div>

        <div style={cardStyle}>
          <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', marginBottom: 18 }}>Haftalik davomat trendi</p>
          <ResponsiveContainer width="100%" height={280}>
        ...
        <div style={cardStyle}>
          <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', marginBottom: 18 }}>Keldi / Kelmadi solishtirma</p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData} barCategoryGap="30%">
              ...
              <Bar dataKey="Keldi" fill="#10B981" radius={[4, 4, 0, 0]} />
              <Bar dataKey="Kelmadi" fill="#F87171" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
  ...
  const renderMonthly = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          <KpiCard label="Jami" value={ov.total || 0} color="#0F172A" borderColor="#E2E8F0" />
          <KpiCard label="Keldi" value={ov.present || 0} color="#059669" borderColor="#A7F3D0" />
          <KpiCard label="Kelmadi" value={ov.absent || 0} color="#DC2626" borderColor="#FECACA" />
          <KpiCard label="Ko'rsatkich" value={`${ov.attendance_rate || 0}%`} color="#4F46E5" borderColor="#C7D2FE" />
        </div>
        ...
        {weekData.length > 0 && (
          <div style={cardStyle}>
            <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', marginBottom: 18 }}>Haftalik ko'rsatkich ({MONTHS[month - 1]})</p>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={weekData}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 12, paddingTop: 10 }} />
                <Line type="monotone" dataKey="Ko'rsatkich" stroke="#4F46E5" strokeWidth={2.5} dot={{ r: 5, fill: '#4F46E5', strokeWidth: 2, stroke: 'white' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        {(data.class_breakdown || []).length > 0 && (
          <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid #F1F5F9' }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>Sinflar kesimida oylik natija</p>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {["Sinf", "Jami", "Keldi", "Kech keldi", "Kelmadi", "Ko'rsatkich"].map(h => (
  ...
  return (
    <div style={{ maxWidth: 1100, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0F172A', margin: 0 }}>Hisobotlar</h1>
          <p style={{ fontSize: 13, color: '#64748B', marginTop: 4 }}>Davomad tahlili va statistikasi</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <Button variant="ghost" size="sm" icon={RefreshCw} onClick={loadData} loading={loading}>Yangilash</Button>
          <Button size="sm" icon={Download} onClick={handleExport} loading={exporting}>Excel</Button>
        </div>
      </div>
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {tab === 'weekly' && (
          <>
            <span style={{ fontSize: 13, color: '#64748B' }}>Hafta oxiri:</span>
            <input type="date" value={selectedDate}
              onChange={e => setSelectedDate(e.target.value)}
              max={format(new Date(), 'yyyy-MM-dd')}
              style={inputStyle}
      ...
      {loading ? (
        ...
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden' }}>
          <EmptyState icon={BarChart3} title="Ma'lumot topilmadi" description="Tanlangan davr uchun hisobot mavjud emas" />
        </div>
      ) : (
