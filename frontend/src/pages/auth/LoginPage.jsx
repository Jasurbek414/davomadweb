...
const features = [
  { icon: Scan,         title: 'Face ID Kirish',     desc: 'Biometric qurilmalar bilan avtomatik davomad' },
  { icon: GraduationCap, title: "O'quvchilar Baza",  desc: 'Barcha maktab, sinf va o\'quvchilarni boshqarish' },
  { icon: Monitor,      title: 'Real-Vaqt Tahlil',   desc: 'Har bir qurilmadan onlayn davomad hisoboti' },
  { icon: Bell,         title: 'Ota-ona Xabarlari',  desc: "Telegram orqali farzand kiriش-chiqishi haqida xabar" },
]
...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    if (login.fulfilled.match(res)) {
      ...
      const e = res.payload
      toast.error(e?.detail || e?.non_field_errors?.[0] || 'Login yoki parol noto\'g\'ri')
    }
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: '#0F172A', overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 56 }}>
          ...
          <div>
            <p style={{ color: 'white', fontWeight: 800, fontSize: 18, lineHeight: 1.2 }}>Maktab Davomad</p>
            <p style={{ color: '#818CF8', fontSize: 12, marginTop: 2 }}>Face ID Tizimi</p>
          </div>
        </div>
        ...
        <p style={{ fontSize: 15, color: '#94A3B8', lineHeight: 1.7, marginBottom: 48, maxWidth: 400 }}>
          Face ID texnologiyasi orqali maktab davomatini avtomatlashtiring.
          Ota-onalar Telegram orqali real vaqtda xabardor bo'ladi.
        </p>
        ...
        <p style={{ color: '#334155', fontSize: 12, marginTop: 48 }}>
          © 2025 Maktab Davomad · Barcha huquqlar himoyalangan
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: 400, animation: 'fadeInUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 40 }}>
            ...
            <div>
              <p style={{ fontSize: 17, fontWeight: 800, color: '#0F172A' }}>Maktab Davomad</p>
              <p style={{ fontSize: 12, color: '#94A3B8' }}>Face ID Tizimi</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            >
              {isLoading
                ? <><Loader2 style={{ width: 17, height: 17, animation: 'spin 0.75s linear infinite' }} /> Kirish...</>
                : 'Kirish →'
              }
            </button>
          </form>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.6, margin: 0 }}>
              Tizimga kirish uchun administrator tomonidan berilgan telefon raqam va paroldan foydalaning.
            </p>
          </div>
